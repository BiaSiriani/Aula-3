...
app.delete("/users/:id", async (req, res) => {
    ...
    //converte id em numero
    try {
        const resultado = await userService.deleteUser(id) 
        //tenta excluir o usuario
        if (!resultado){
            return res.status(406).json({"Mensagem": "Usuario não existe"});
        }
...
app.put("/users/:id", async (req, res) => {
    const id = parseInt(req.params.id);
    const { nome, email, senha, endereco, telefone, cpf, idade } = req.body;
    try {
        const resultado = await userService.updateUser(id, nome, email, senha, endereco, telefone, cpf, idade);
        if (!resultado) {
